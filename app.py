...
# --- 路径与文件配置 ---
DATA_DIR = "/app/data"
LOG_DIR = os.path.join(DATA_DIR, "log")      # 日志专用文件夹
...
def write_log_csv(data_dict):
    """
    按天生成 CSV 日志：data/log/log_YYYY-MM-DD.csv
    """
...
def test_single_channel(sub_id, name, url, use_hw):
    ...
    log_entry = {"时间": get_now(), "所属任务": status['sub_name'], "探测状态": "失败", "频道名称": name, "完整URL": url}
    ...
    try:
        ...
        log_entry.update({
            "探测状态": "成功", "分辨率": meta['res'], "视频编码": meta['v_codec'], "音频编码": meta['a_codec'],
            "帧率(FPS)": meta['fps'], "延迟(ms)": latency, "网速(Mbps)": speed, "地区": city, "运营商": isp
        })
...
def run_task(sub_id):
    ...
    start_ts = time.time()
    subs_status[sub_id] = {"running": True, "stop_requested": False, "total": 0, "current": 0, "success": 0, "sub_name": sub['name'], "logs": [], "summary_host": {}, "summary_city": {}, "consecutive_failures": {}, "blacklisted_hosts": set(), "analytics": {"res": {}, "lat": {}}}
    ...
    subs_status[sub_id]["logs"].append(f"🎬 任务开始: {get_now()} | 源数量: {len(raw_channels)} | 硬件加速: {'ON' if use_hw else 'OFF'}")
    valid_list = []
    ...
    try:
        status["logs"].append(" "); status["logs"].append("📊 --- 接口汇总报告 ---")
    ...
    status["logs"].append(f"⏰ 更新时间: {update_ts} | ⌛ 总耗时: {duration} | 🏁 结算完毕 (有效源: {len(valid_list)})")
