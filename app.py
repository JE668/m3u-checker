        ...
        try:
            ...
            if r.returncode == 0:
                ...
                if v.get('avg_frame_rate') and '/' in v['avg_frame_rate']:
                    try: num, den = v['avg_frame_rate'].split('/'); fps = str(round(int(num)/int(den))) if int(den)>0 else "?"
                    except: pass
...
def test_single_channel(sub_id, name, url, use_hw):
    ...
    try:
        ...
        with log_lock:
            ...
            h = int(meta['h']); r_tag = "8k" if h>=4320 else "4k" if h>=2160 else "1080p" if h>=1080 else "720p" if h>=720 else "sd"
            ...
            score = h + speed*10 - latency/10
            msg = f"✅ {name}: {meta['icon']}{meta['res']} | 🎬{meta['v_codec']} | 🎵{meta['a_codec']} | 🎞️{meta['fps']} | 📊{speed}Mbps | ⏱️{latency}ms | 📍{city} | 🌐{hp}"
        ...
        return {"name": name, "url": url, "score": score, "res_tag": r_tag.lower()}
...
def run_task(sub_id):
    ...
    subs_status[sub_id] = {"running": True, "stop_requested": False, "total": 0, "current": 0, "success": 0, "sub_name": sub['name'], "logs": [], "summary_host": {}, "summary_city": {}, "consecutive_failures": {}, "blacklisted_hosts": set(), "analytics": {"res": {"SD":0,"720P":0,"1080P":0,"4K":0,"8K":0}, "lat": {"<100ms":0,"<500ms":0,">500ms":0}, "codec": {}, "stability": {"success":0, "fail":0, "banned":0}}, "perf": {"total_lat": 0, "total_speed": 0}}
    ...
    try:
        ...
        for line in r.text.split('\n'):
            line = line.strip()
            if "#EXTINF" in line: current_name = line.split(',')[-1].strip()
            elif line.startswith("http"): raw_channels.append((current_name, line))
            elif "," in line and "http" in line:
                p = line.split(','); raw_channels.append((p[0].strip(), p[1].strip()))
    except Exception as e:
        subs_status[sub_id]["logs"].append(f"❌ 订阅源读取失败: {e}"); subs_status[sub_id]["running"] = False; return
    ...
    subs_status[sub_id]["logs"].append(f"🎬 任务开始: {get_now()} | 总数: {total_num} | 线程: {thread_num}")
    ...
    try:
        ...
        status["logs"].append(f"✅ 文件生成成功: {sub_id}.m3u / .txt")
    except Exception as e:
        status["logs"].append(f"❌ 文件保存失败: {e}")

    # 报告生成
    status["logs"].append(" "); status["logs"].append("📜 ==================== 探测结算报告 ====================")
    status["logs"].append(f"⏱️ 任务总耗时: {format_duration(duration)} | 有效源: {len(valid_list)} / 成功探测: {status['success']}")
    ...
    status["logs"].append("📡 --- 接口质量全量统计 (按效率排序) ---")
...
def hw_test():
    try:
        ...
        return jsonify({"status": "success" if ready else "error", "message": "✅ GPU加速就绪" if ready else "❌ 驱动连接异常", "codecs": codecs, "raw": out})
    except Exception as e: return jsonify({"status": "error", "raw": str(e)})
@app.route('/api/status/<sub_id>')
def get_status(sub_id):
    info = subs_status.get(sub_id, {"running": False, "logs": [], "total":0, "current":0, "success":0, "analytics": {"res":{},"lat":{},"codec":{},"stability":{"success":0,"fail":0,"banned":0}}})
