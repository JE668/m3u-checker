...
def test_single_channel(sub_id, name, url, use_hw):
    ...
    try:
        ...
        with log_lock:
            ...
            status["analytics"]["res"][res_tag.upper()] += 1
...
def delete_sub(sub_id): config = load_config(); config["subscriptions"] = [s for s in config["subscriptions"] if s["id"] != sub_id]; save_config(config); update_global_scheduler(); return jsonify({"status": "ok"})
