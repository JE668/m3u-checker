        ...
        with log_lock:
            ...
            if status["consecutive_failures"][hp] >= 10:
                if hp not in status["blacklisted_hosts"]: 
                    status["blacklisted_hosts"].add(hp); status["logs"].append(f"⚠️ 熔断激活: 接口 {hp} 连续失败10次，已封禁。")
...
def run_task(sub_id):
    ...
    status["logs"].append(f"⏱️ 总计耗时: {duration} | 有效源: {len(valid_list)} / 成功探测: {status['success']}")
...
def get_status(sub_id):
    ...
    return jsonify({"running": info["running"], "logs": info["logs"][-150:], "total": info["total"], "current": info["current"], "success": info["success"], "banned_count": len(info.get("blacklisted_hosts", [])), "analytics": info["analytics"]})
