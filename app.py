...
def probe_stream(url, use_hw):
    ...
    base_cmd = [
        ...
        '-user_agent', 'Mozilla/5.0', '-probesize', '5000000', '-analyzeduration', '5000000'
    ]
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            return {"res": f"{v.get('width','?')}x{v.get('height','?')}", "v_codec": v.get('codec_name', 'UNK').upper(), "a_codec": a.get('codec_name', 'UNK').upper(), "fps": f"{fps}fps", "br": br, "icon": icon}
    except: pass
    return None
    ...
    try:
        ...
        with log_lock:
            ...
            if not status["stop_requested"]:
                ...
                msg = f"{meta['icon']}{meta['res']} | 🎬{meta['v_codec']} | 🎵{meta['a_codec']} | 🎞️{meta['fps']} | 📊{meta['br']} | ⏱️{latency}ms | 🚀{speed}Mbps | 📍{city} | 🔌{hp}"
        ...
        with log_lock:
            ...
            if not status["stop_requested"]: status["logs"].append(f"❌ {name}: 探测失败({str(e)}) | 🔌{hp}")
        return None
    finally:
...
def run_task(sub_id):
    ...
    start_ts = time.time()
    subs_status[sub_id] = {"running": True, "stop_requested": False, "total": 0, "current": 0, "success": 0, "sub_name": sub['name'], "logs": [], "summary_host": {}, "summary_city": {}, "consecutive_failures": {}, "blacklisted_hosts": set()}
    ...
    status["logs"].append(f"🏁 结算完毕 (有效源: {len(valid_list)}) | 总耗时: {duration_str}")
