...
def test_single_channel(sub_id, name, url, use_hw):
    ...
    try:
        ...
        return {"name": name, "url": url, "score": h_v + speed*5 - latency/10, "res_tag": r_tag.lower()}
...
def run_task(sub_id):
    ...
    subs_status[sub_id] = {"running": True, "stop_requested": False, "total": 0, "current": 0, "success": 0, "sub_name": sub['name'], "logs": [], "summary_host": {}, "summary_city": {}, "consecutive_failures": {}, "blacklisted_hosts": set(), "analytics": {"res": {"SD":0,"720P":0,"1080P":0,"4K":0,"8K":0}, "lat": {"<100ms":0,"<500ms":0,">500ms":0}}}
    ...
    status["logs"].append(f"⏱️ 任务耗时: {format_duration(time.time()-start_ts)} | 有效源: {len(valid_list)} / {status['success']}")
    
    # 分辨率统计输出
    status["logs"].append("📉 --- 分辨率统计分布 ---")
    for r, c in status["analytics"]["res"].items():
        if c > 0: status["logs"].append(f"📺 {r:<10} | 探测总数: {c}")

    status["logs"].append(" "); status["logs"].append("📡 --- 接口质量汇总 ---")
    sh = sorted([i for i in status["summary_host"].items() if i[1]['t']>0], key=lambda x: x[1]['s']/x[1]['t'], reverse=True)
    for h, d in sh[:10]: status["logs"].append(f"⭐️ {h:<28} | 有效率: {round(d['s']/d['t']*100, 1)}% ({d['s']}/{d['t']})")
    status["logs"].append("======================================================")
...
def get_status(sub_id):
    ...
    return jsonify({"running": s["running"], "logs": s["logs"][-150:], "total": s["total"], "current": s["current"], "success": s["success"], "analytics": s["analytics"]})
