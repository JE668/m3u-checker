...
def test_single_channel(sub_id, name, url, use_hw):
    ...
    try:
        ...
        # 3. IP 定位 (单独 try 保护，防止定位超时导致频道判定失败)
...
def run_task(sub_id):
    ...
    # 预先获取唯一 Host 进行 IP 缓存填充 (防止并发请求阻塞 ip-api)
    unique_hosts = list(set([urlparse(c[1]).hostname for c in raw_channels if c[1]]))
    subs_status[sub_id]["logs"].append(f"🔍 预检完成，检测到 {len(unique_hosts)} 个独立节点地址...")
